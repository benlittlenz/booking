...
function Register(props: any): ReactElement {
  ...
  const submit = (): Promise<any> => {
    ...
    const inputErrors: boolean | { name: string; email: string; password: string } =
      userValidator.validateRegistrationInput(name, email, password, password_confirmed, 8);
      ...
      <div className="absolute w-full md:w-3/5 lg:w-1/3 top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2">
        ...
        >
          <>
            ...
            {props.registerError && <Alert type="danger">{props.registerError}</Alert>}
            ...
            <div className="w-full flex mt-3 text-blue-500">
              <Link href="/user/login">
                <a className="text-xs underline">Already have an account?</a>
              </Link>
            </div>
          </>
        </Card>
